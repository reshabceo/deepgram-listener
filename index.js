...
app.ws('/listen', async (plivoWs, req) => {
  ...
  try {
    ...
    // Store the audio file info in Supabase for the XML handler to use
    await supabase.from('call_audio').insert([{
      call_uuid: callId,
      turn: 1,
  ...
  dgConn.on(LiveTranscriptionEvents.Transcript, async data => {
    ...
    if (data.is_final && textUtils.isEndOfThought(ctx.transcriptBuffer, elapsed)) {
      ...
      // Generate TTS audio file and store info for XML handler
      try {
        const audioFile = await generateTTSFile(aiText, callId);
        const currentTurn = ctx.turnCount + 1;
        
        // Store the audio file info in Supabase
        await supabase.from('call_audio').insert([{
          call_uuid: callId,
          turn: currentTurn,
          audio_file: audioFile,
        ...
        // Update context turn count
        ctx.turnCount = currentTurn;
      } catch (error) {
